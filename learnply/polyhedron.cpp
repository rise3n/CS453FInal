...
Polyhedron::Polyhedron(FILE *file)
{
	...
	for (i = 0; i < in_ply->num_elem_types; i++) {
		...
		if (equal_strings("vertex", elem_name)) {
			...
			for (j = 0; j < elem_count; j++) {
				...
				qlist[j] = new Quad;
				qlist[j]->verts[0] = (Vertex *)(intptr_t)face.verts[0];
				qlist[j]->verts[1] = (Vertex *)(intptr_t)face.verts[1];
				qlist[j]->verts[2] = (Vertex *)(intptr_t)face.verts[2];
				qlist[j]->verts[3] = (Vertex *)(intptr_t)face.verts[3];
	...
	for (i = 0; i < nquads; i++) {
		qlist[i]->verts[0] = vlist[(intptr_t)qlist[i]->verts[0]];
		qlist[i]->verts[1] = vlist[(intptr_t)qlist[i]->verts[1]];
		qlist[i]->verts[2] = vlist[(intptr_t)qlist[i]->verts[2]];
		qlist[i]->verts[3] = vlist[(intptr_t)qlist[i]->verts[3]];
	}
